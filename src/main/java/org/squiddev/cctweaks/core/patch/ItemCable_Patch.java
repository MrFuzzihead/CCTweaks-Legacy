...
public class ItemCable_Patch extends ItemCable implements TItemMultiPart {
    ...
    public TMultiPart newPart(ItemStack stack, EntityPlayer player, World world, BlockCoord pos, int side,
        ...
        switch (getPeripheralType(stack)) {
            ...
            case WiredModem:
                return new PartModem(Facing.oppositeSide[side]);
        }
    ...
    /**
     * The original {@link ItemCable#onItemUse(ItemStack, EntityPlayer, World, int, int, int, int, float, float, float)}
     * method
    ...
    public boolean nativePlace(ItemStack stack, EntityPlayer player, World world, int x, int y, int z, int side,
        float hitX, float hitY, float hitZ) {
        return false;
    }
