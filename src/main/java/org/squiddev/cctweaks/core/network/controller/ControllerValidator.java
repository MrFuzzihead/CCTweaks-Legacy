    ...
    public static void validate(NetworkController controller) {
        ...
        for (Map.Entry<INetworkNode, Point> entry : controller.points.entrySet()) {
            ...
            if (point.node.getAttachedNetwork() != controller) {
                errors.add(
                    String.format(
                        "Controller for node %s: %s != %s",
                        point.node,
                        point.node.getAttachedNetwork(),
                        controller));
            }
            ...
            for (Map.Entry<String, IPeripheral> peripheral : point.peripherals.entrySet()) {
                ...
                if (other == null || !peripheral.getValue()
                    .equals(other)) {
                    String error = String.format(
                        "Peripherals for node %s (%s): %s != %s",
                        point.node,
                        peripheral.getKey(),
                        peripheral.getValue(),
                        other);
        ...
        for (Map.Entry<String, IPeripheral> peripheral : controller.getPeripheralsOnNetwork()
            ...
            if (!foundPeripherals.contains(peripheral.getValue())) {
                errors.add(
                    String.format("Peripheral not in any node %s => %s", peripheral.getKey(), peripheral.getValue()));
            }
        }
