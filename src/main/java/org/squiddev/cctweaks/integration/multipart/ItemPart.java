...
public class ItemPart extends ItemBase {
    ...
    public boolean onItemUse(ItemStack stack, EntityPlayer player, World world, int x, int y, int z, int side,
        ...
        if (hitDepth < 1 && MultipartHelpers.place(world, pos, getPart(stack.getItemDamage(), side)) || MultipartHelpers
            .place(world, pos.offset(side), getPart(stack.getItemDamage(), Facing.oppositeSide[side]))) {
    ...
    public static IIcon getIcon(int damage, int side) {
        switch (damage) {
            case 0:
                return PartWirelessBridge.getRenderer()
                    .getBlockIconFromSide(Registry.blockNetworked, side);
            default:
                return null;
        }
    }
