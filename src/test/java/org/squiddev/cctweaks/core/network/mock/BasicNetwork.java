...
public class BasicNetwork implements IBlockAccess, Iterable<Map.Entry<BlockCoord, KeyedNetworkNode>> {
    ...
    public void dump() {
        SetMultimap<INetworkController, BlockCoord> networks = MultimapBuilder.hashKeys()
            .hashSetValues()
            .build();
        ...
        for (Map.Entry<INetworkController, Collection<BlockCoord>> entry : networks.asMap()
            .entrySet()) {
