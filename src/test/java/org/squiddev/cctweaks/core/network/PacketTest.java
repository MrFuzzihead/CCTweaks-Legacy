...
public class PacketTest {
    ...
    public static Object[][] data() {
        TestData[] data = new Gson()
            .fromJson(new InputStreamReader(PacketTest.class.getResourceAsStream("data.json")), TestData[].class);

        Object[][] result = new Object[data.length][];
        for (int i = 0; i < data.length; i++) {
            TestData item = data[i];
            result[i] = new Object[] { item.name, item };
        }
    ...
    public void testCounts() {
        ...
        network.reset();
        ((IWorldNetworkNodeHost) network.getTileEntity(0, 0, 0)).getNode()
            .getAttachedNetwork()
            .invalidateNetwork();
    ...
    public void testDistance() {
        ...
        node.getAttachedNetwork()
            .transmitPacket(node, new Packet(0, 0, null, null));
