...
public final class NetworkBindings {
    ...
    private static final SetMultimap<UUID, IWorldNetworkNode> uuidNetworks = MultimapBuilder.hashKeys()
        .hashSetValues()
        .build();
    private static final SetMultimap<Integer, IWorldNetworkNode> idNetworks = MultimapBuilder.hashKeys()
        .hashSetValues()
        .build();
        ...
        if (nodes.add(node)) {
            for (IWorldNetworkNode n : nodes) {
                if (!n.equals(node) && n.getAttachedNetwork() != null) {
                    n.getAttachedNetwork()
                        .formConnection(n, node);
                }
            }
        }
    }
