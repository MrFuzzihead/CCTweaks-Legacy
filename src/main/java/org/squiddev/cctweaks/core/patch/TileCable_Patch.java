...
@MergeVisitor.Rename(
    from = "dan200/computercraft/shared/peripheral/modem/TileCable$Packet",
    to = "org/squiddev/cctweaks/api/network/Packet")
            ...
            return modem = new DirectionalPeripheralModem() {
                ...
                protected boolean isPeripheralEnabled() {
                    return super.isPeripheralEnabled() && !m_destroyed
                        && getPeripheralTypeSafe() == PeripheralType.WiredModemWithCable;
                }
            };
        }
        return modem;
    ...
    public void onNeighbourChange() {
        ...
        if (type == PeripheralType.WiredModemWithCable) {
            if (getModem().updateEnabled()) {
                modem.getAttachedNetwork()
                    .invalidateNode(modem);
                updateAnim();
            }
        }
    }
