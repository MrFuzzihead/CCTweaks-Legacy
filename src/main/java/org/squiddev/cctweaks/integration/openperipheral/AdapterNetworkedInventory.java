...
public class AdapterNetworkedInventory implements IPeripheralAdapter {
    ...
    private static IInventory getInventory(INetworkAccess network, String name) {
        Object object = CCTweaksAPI.instance()
            .peripheralHelpers()
            .getTarget(
                network.getPeripheralsOnNetwork()
                    .get(name));
    ...
    public int pullItemRemote(IInventory target, @Env(IPeripheralEnvironments.ARG_NETWORK) INetworkAccess network,
        ...
        @Optionals @Arg(
            name = "maxAmount",
            description = "The maximum amount of items you want to pull") Integer maxAmount,
        @Arg(
            name = "fromDirection",
            description = "The direction to pull from the OTHER inventory") ForgeDirection fromDirection,
        @Arg(
            name = "intoSlot",
            description = "The slot in the current inventory that you want to pull into") Integer intoSlot,
        @Arg(
            name = "intoDirection",
            description = "The direction to push into the current inventory") ForgeDirection intoDirection) {
        Preconditions.checkNotNull(network, "Cannot find the network");

        final IInventory otherInventory = Preconditions
            .checkNotNull(getInventory(network, remoteName), "Other inventory not found");
        final IInventory thisInventory = Preconditions
            .checkNotNull(InventoryUtils.getInventory(target), "Inventory not found");
        ...
        final int amount = InventoryUtils
            .moveItemInto(otherInventory, fromSlot, fromDirection, thisInventory, intoSlot, intoDirection, maxAmount);
        if (amount > 0) {
    ...
    public int pushItemRemote(IInventory target, @Env(IPeripheralEnvironments.ARG_NETWORK) INetworkAccess network,
        @Arg(name = "remoteName", description = "The name of the remote inventory") String remoteName,
        @Arg(
            name = "fromSlot",
            description = "The slot in the current inventory that you're pushing from") int fromSlot,
        @Optionals @Arg(
            name = "maxAmount",
            description = "The maximum amount of items you want to push") Integer maxAmount,
        @Arg(
            name = "fromDirection",
            description = "The direction to push into the current inventory") ForgeDirection fromDirection,
        @Arg(
            name = "intoSlot",
            description = "The slot in the other inventory that you want to push into") Integer intoSlot,
        @Arg(
            name = "intoDirection",
            description = "The slot in the other inventory that you're pulling from") ForgeDirection intoDirection) {
        Preconditions.checkNotNull(network, "Cannot find the network");
        ...
        int amount = InventoryUtils
            .moveItemInto(thisInventory, fromSlot, fromDirection, otherInventory, intoSlot, intoDirection, maxAmount);
        if (amount > 0) {
            thisInventory.markDirty();
            otherInventory.markDirty();
        }
