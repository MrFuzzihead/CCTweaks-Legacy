    ...
    private void renderNetwork(VisualisationData data, Color color, float thickness) {
        ...
        for (Connection connection : data.connections) {
            ...
            if (position.typeOfHit == MovingObjectPosition.MovingObjectType.BLOCK) {
                if (a.position != null) {
                    ...
                    if (nodePosition.x == position.blockX && nodePosition.y == position.blockY
                        && nodePosition.z == position.blockZ) {
                        nodes.add(a);
        ...
        if (data.connections.length == 0 && position.typeOfHit == MovingObjectPosition.MovingObjectType.BLOCK) {
            for (Node node : data.nodes) {
                ...
                if (nodePosition != null && nodePosition.x == position.blockX
                    && nodePosition.y == position.blockY
                    && nodePosition.z == position.blockZ) {
                    nodes.add(node);
                }
            }
