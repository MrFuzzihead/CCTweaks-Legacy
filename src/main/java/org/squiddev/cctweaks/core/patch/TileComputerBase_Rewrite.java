...
/**
 * Complete rewrite of {@link dan200.computercraft.shared.computer.blocks}
 */
public abstract class TileComputerBase_Rewrite extends TileGeneric implements IComputerTile, IDirectionalTile {
    ...
    protected boolean m_fresh = false;

    protected TileComputerBase_Rewrite() {
        super();
    }
    ...
    protected boolean onDefaultComputerInteract(EntityPlayer player) {
        if (!this.worldObj.isRemote && this.isUsable(player, false)) {
            this.createServerComputer()
                .turnOn();
            this.openGUI(player);
        }
    ...
    public int getRedstoneOutput(int side) {
        int localDir = this.remapLocalSide(DirectionUtil.toLocal(this, side));
        if (!this.isRedstoneBlockedOnSide(localDir) && !this.worldObj.isRemote) {
    ...
    public void updateEntity() {
        if (!this.worldObj.isRemote) {
            ...
            if (computer != null) {
                if (this.m_startOn || (m_fresh && m_on)) {
                    computer.turnOn();
                ...
                for (int dir = 0; dir < 6; dir++) {
                    ...
                    if (!this.isRedstoneBlockedOnSide(localDir)) {
                        computer.setRedstoneInput(
                            localDir,
                            RedstoneUtil.getRedstoneOutput(this.worldObj, offsetX, offsetY, offsetZ, offsetSide));
                        computer.setBundledRedstoneInput(
                            localDir,
                            RedstoneUtil
                                .getBundledRedstoneOutput(this.worldObj, offsetX, offsetY, offsetZ, offsetSide));
                    }

                    if (!this.isPeripheralBlockedOnSide(localDir)) {
                        computer.setPeripheral(
                            localDir,
                            PeripheralUtil.getPeripheral(this.worldObj, offsetX, offsetY, offsetZ, offsetSide));
                    }
                }
            }
        }
    ...
    public void setComputerID(int id) {
        if (!this.worldObj.isRemote && this.m_computerID != id) {
            this.m_computerID = id;
            ServerComputer computer = this.getServerComputer();
            if (computer != null) {
    ...
    public IComputer createComputer() {
        return (IComputer) (this.worldObj.isRemote ? this.createClientComputer() : this.createServerComputer());
    }

    @Override
    public IComputer getComputer() {
        return (IComputer) (this.worldObj.isRemote ? this.getClientComputer() : this.getServerComputer());
    }

    @Override
    public ComputerFamily getFamily() {
        return this.getBlock()
            .getFamily(this.worldObj, this.xCoord, this.yCoord, this.zCoord);
    }
    ...
    public ServerComputer getServerComputer() {
        return !this.worldObj.isRemote ? ComputerCraft.serverComputerRegistry.get(this.m_instanceID) : null;
    }

    public ClientComputer createClientComputer() {
        if (this.worldObj.isRemote && this.m_instanceID >= 0) {
    ...
    public ClientComputer getClientComputer() {
        return this.worldObj.isRemote ? ComputerCraft.clientComputerRegistry.get(this.m_instanceID) : null;
    }
    ...
    protected void transferStateFrom(TileComputerBase_Rewrite copy) {
