...
public final class NetworkHelpers implements INetworkHelpers {
    ...
    public boolean canConnect(IBlockAccess world, int x, int y, int z, ForgeDirection direction) {
        ...
        IWorldNetworkNode node = NetworkAPI.registry()
            .getNode(world, x, y, z);
    ...
    public Set<INetworkNode> getAdjacentNodes(IWorldNetworkNode node, boolean checkExists) {
        ...
        for (ForgeDirection direction : ForgeDirection.VALID_DIRECTIONS) {
            if (node.canConnect(direction)) {
                int x = position.getX() + direction.offsetX, y = position.getY() + direction.offsetY,
                    z = position.getZ() + direction.offsetZ;
                if (world == null || world.blockExists(x, y, z)) {
                    IWorldNetworkNode neighbour = NetworkAPI.registry()
                        .getNode(access, x, y, z);
    ...
    public void joinOrCreateNetwork(INetworkNode node, Set<? extends INetworkNode> connections) {
        ...
        if (node.getAttachedNetwork() == null) {
            joinNewNetwork(node);
            for (INetworkNode neighbour : connections) {
                node.getAttachedNetwork()
                    .formConnection(node, neighbour);
            }
        }
    }
    ...
    public void scheduleJoin(final IWorldNetworkNode node, final TileEntity tile) {
        ...
        FmlEvents.schedule(new Runnable() {
            ...
            public void run() {
                ...
                if (world != null && world.getTileEntity(tile.xCoord, tile.yCoord, tile.zCoord) == tile) {
                    joinOrCreateNetwork(node);
                }
            }
        });
    ...
    public static void scheduleConnect(final AbstractWorldNode node, final TMultiPart part) {
        if (node == null) throw new IllegalArgumentException("node cannot be null");
        FmlEvents.schedule(new Runnable() {
